...
export const createDeposit = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (paymentMethod.code.toLowerCase() === 'crypto') {
  ...
  } else if (paymentMethod.code.toLowerCase() === 'ggusonepay') {
    const { ggusonepayMethod } = req.body
    if (!ggusonepayMethod) {
      throw new AppError('Payment method selection is required', 400)
    }
    ...
    try {
      ...
      await prisma.deposit.update({
        where: { id: deposit.id },
        data: { webhookData: { provider: 'ggusonepay', status: 'awaiting_payment', method: ggusonepayMethod, orderRes } }
      })
      ...
      return res.status(201).json({
        ...
        data: { 
          ...
          paymentUrl: orderRes.data?.cashierUrl || null,
          ggusonepayData: orderRes.data
        }
      })
    } catch (err: any) {
