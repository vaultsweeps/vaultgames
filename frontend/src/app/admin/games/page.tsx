    ...
    <div className="space-y-5">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Manage your games library and downloads.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchGames} className="glass border border-white/10 rounded-xl px-3 py-2.5 text-slate-400 hover:text-white transition-all">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={game.id}>
                  ...
                  <td className="text-slate-400 text-sm">{game.category}</td>
                  ...
                  <td>
                    <div className="flex gap-1">
                      <button onClick={() => toggleActive(game)}
                        className={`w-7 h-7 rounded-lg flex items-center justify-center border transition-all ${game.isActive ? 'text-green-400 bg-green-500/10 border-green-500/20' : 'text-slate-500 glass border-white/10 hover:text-white'}`}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">Game Name *</label>
                ...
                <div>
                  ...
                  <select value={editing.category || 'Action'} onChange={e => setEditing((p: any) => ({ ...p, category: e.target.value }))} className="input-neon bg-dark-800">
                    {CATEGORIES.map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">Version</label>
              ...
              <div>
                <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">Download URL</label>
              ...
              <div>
                <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">Thumbnail URL</label>
              ...
              <div>
                <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">System Requirements</label>
              ...
              <div>
                <label className="block text-xs font-mono tracking-wider text-slate-400 uppercase mb-2">Installation Instructions</label>
